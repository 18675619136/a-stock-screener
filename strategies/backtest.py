#!/usr/bin/env python3
"""
Backtester for momentum_ma v2 strategy — CLOSE>MA5>MA18 + 放量 + 动态卖出.

Buy:
  - CLOSE > MA5 > MA18
  - Volume > 18-day avg × 1.2
  - MV < 1000亿, 总股本 0.5~10亿

Sell (逐日监控):
  1. price < buy_price × 0.94 → 卖出全部 (hard stop loss)
  2. price < MA18 → 卖出全部 (hard stop)
  3. price > MA5 × 1.3 → 卖出一半 (take profit)
  4. price < MA5 → 卖出一半 (trailing stop)

Usage:
    python3 -m strategies.backtest --universe 500 --save
"""
...
class BacktestResult:
    ...
    def summary(self) -> str:
        lines = [
            "╔══════════════════════════════════════════════╗",
            "║   momentum_ma v2 Backtest (动态卖出)          ║",
            "╠══════════════════════════════════════════════╣",
            f"║  Period: {self.date_labels[0] if self.date_labels else 'N/A'} → {self.date_labels[-1] if self.date_labels else 'N/A'}  ║",
    ...
    def to_dict(self) -> dict:
        ...
        if hasattr(self, 'sharpe_ratio'):
            d["sharpe_ratio"] = round(self.sharpe_ratio, 2)
...
class MomentumMABacktester:
    ...
    def fetch_universe(self) -> tuple[list[dict], dict[str, dict]]:
        ...
        for s in filtered:
            ...
            if mv <= 0 or mv > self.cfg["max_mv"]:
                continue
            if total_shares <= 0 or total_shares < self.cfg["min_total_shares"] or total_shares > self.cfg["max_total_shares"]:
                continue
            if is_st(name):
                continue
            enriched.append({
    ...
    def check_sell_conditions(
        ...
        ma18 = calc_ma(closes, self.cfg["ma_long"])
    ...
    def run(self) -> BacktestResult:
        ...
        # 6. Close remaining positions at last available date
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Backtest momentum_ma v2 (CLOSE>MA5>MA18 + volume + dynamic sell)"
    )
