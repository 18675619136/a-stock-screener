#!/usr/bin/env python3
"""
Backtester for bull_ma_limitup strategy — 价格>MA5>MA18 + 近20日涨幅>8% + 动态卖出.
...
Usage:
    python3 -m strategies.backtest_bull_ma_limitup --help
    python3 -m strategies.backtest_bull_ma_limitup --universe 1000 --save
"""
...
class BacktestResult:
    ...
    def summary(self) -> str:
        lines = [
            "╔══════════════════════════════════════════════╗",
            "║   bull_ma_limitup Backtest (MA多头+涨停)     ║",
            "╠══════════════════════════════════════════════╣",
...
# ── Position tracking (same as dual_ma_gc) ─────────────────────────
...
class BullMALimitUpBacktester:
    ...
    def fetch_universe(self) -> tuple[list[dict], dict[str, dict]]:
        ...
        log("▶ Fetching market data (MV, total shares)...")
    ...
    def run_strategy_at_date(
        ...
        for code, klines in klines_data.items():
            ...
            ma18 = sum(closes[-ma_l:]) / ma_l

            # 1. close > MA5 > MA18
            if not (close > ma5 > ma18):
                continue

            # 2. Check last N days for surge > 8%
            has_surge = False
            surge_date = ""
            check_len = min(lookback + 1, n)
            for j in range(1, check_len):
                ...
                if pct >= surge:
                    has_surge = True
                    surge_date = klines[-(j)]["date"] if j <= len(klines) else ""
                    break
    ...
    def check_sell_conditions(
        ...
        """Same dynamic sell logic as dual_ma_gc."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Backtest bull_ma_limitup (MA多头+涨停+动态卖出) strategy",
    )
    ...
    parser.add_argument("--lookback", type=int, default=20,
                        help="Lookback days for surge detection")
    parser.add_argument("--surge", type=float, default=8.0,
                        help="Surge threshold in %")
    ...
    bt = BullMALimitUpBacktester(bt_config)
    t0 = time.time()
    result = bt.run()
    ...
    if args.save:
        ...
        path = os.path.join(output_dir, "bull_ma_limitup_backtest.json")
